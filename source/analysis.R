...
plot_jail_pop_for_us <- function()  {
  ggplot(get_year_jail_pop(), aes(x = year, y = total_jail_pop)) +
    ...
    labs(caption = "This plot shows the population distribution by time in the United States.")
...
ratio_diff <- df4 %>% summarize(state = State, pop_ratio = Total_pop*100/max(Total_pop))
